...
BASE = subprocess.check_output(['git', '-C', str(ROOT), 'show', '8f61dd5a30b24628317caf5a27544522c221a25e:dev/stage-a-gates.py'], text=True)
...
previous = plan(CURRENT, ['--m1-close'])
current = plan(CURRENT, ['--m2-reference'])
if current[0] != 'M2-REFERENCE' or current[2][:-1] != previous[2] or len(current[2]) != 76:
    raise SystemExit('New gate must append exactly one leg to all prior gates')
if current[1] != sorted(previous[1] + ['ERC20-REFERENCE']):
...
output = ROOT / 'dev/validation/2026-09-22-m2-reference/GATE-COMPATIBILITY.json'
